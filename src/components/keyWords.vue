<template>
    <div class="contentMain">
        ...
        <div class="typeContent">
            <el-row>
                <el-button type="primary" @click="showAddType()">添加关键字</el-button>
            </el-row>
            <el-table
                    :data="tableData"
                    border
        ...
        <el-dialog :title="title" :visible.sync="dialogFormVisible" style="max-width: 1000px;margin: 0 auto;">
            <el-form>
                ...
                <el-form-item label="关键字名称" :label-width="formLabelWidth">
...
<script>
    import {getKeys, checkLogin, addKey, delKey, updateKey} from "../tools/admin";
        ...
        methods: {
            ...
            handleDelete(index, data){
                ...
                this.$confirm('确认删除？删除后将无法恢复！', '确认删除', {
                    ...
                    delKey({'id': data.id}).then(function (data1) {
                        if(data1.status){
            ...
            addTypeNow(){
                let _this = this
                if(_this.type == 'add'){
                    addKey({'name': _this.name}).then(function (data) {
                        if(data.status){
                    ...
                    updateKey({id: _this.id, 'name': _this.name}).then(function (data) {
                        if(data.status){
